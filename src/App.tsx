...
import { BookOpen, Home, Activity, Briefcase, Calendar as CalendarIcon, ChevronLeft as ChevronLeftIcon, ChevronRight as ChevronRightIcon, ShieldCheck, Video as VideoIcon } from 'lucide-react';
...
type View = 'dashboard' | 'matrix' | 'stats' | 'modules' | 'institution' | 'plans' | 'finance' | 'spiritual' | 'notifications' | 'ordem-no-caos' | 'familiar' | 'profissional' | 'biblioteca' | 'fitness' | 'calendar' | 'missions' | 'logistics' | 'messages' | 'profile' | 'admin-panel' | 'video-call';

export default function App() {
  const { user, userData: authUserData, loading: authLoading, logout, profileType, setProfileType, viewMode, setViewMode } = useAuth();
  ...
  // Automatic Institution Creation for Trial
  useEffect(() => {
    const pendingPlan = localStorage.getItem('pending_plan_type');
    if (user && authUserData && !authUserData.institutionId && pendingPlan) {
  ...
  }, [user, authUserData, setProfileType]);
  ...
  useEffect(() => {
    ...
    // Sync All Institutions (Platform Admin only)
    ...
    if (userData?.isPlatformAdmin) {
      ...
      unsubscribeAllInst = onSnapshot(q, (snapshot) => {
  ...
  }, [user, lastRankPosition, userData?.isPlatformAdmin]);
  ...
  const addTask = async (data: any) => {
    ...
    console.log("Criando tarefa para userId:", currentUser.uid);

    try {
      // Ensure user profile exists (fix for new users)
      const userRef = doc(db, 'users', currentUser.uid);
      const userSnap = await getDoc(userRef);
      let currentInstitutionalId = userData?.institutionId || null;
      let currentUserRole = userData?.role || 'MEMBER';
      
      if (!userSnap.exists()) {
        console.log("Perfil não encontrado, criando perfil básico para:", currentUser.uid);
        ...
        // Fallback values for this initial task
        currentUserRole = 'MEMBER';
      } else {
        const data = userSnap.data();
        currentInstitutionalId = data.institutionId || null;
      ...
      const lastTs = (userData?.lastTaskCreatedAt || userSnap.data()?.lastTaskCreatedAt)?.toMillis() || 0;
      const now = Date.now();
      ...
      const needsApproval = isProfessional && currentUserRole === 'MEMBER';
      ...
      const taskData = {
        ...
        assignedBy: isDelegated ? currentUser.uid : null,
        assignedByName: isDelegated ? (currentUser.displayName || 'Admin') : null,
      ...
      // Intelligence: Link to Family Mural if category is familiar
      if (data.moduleId === 'familiar' && data.familiarSubCategory) {
        const familyRef = doc(collection(db, 'personal_family'));
        batch.set(familyRef, {
          userId: currentUser.uid,
          title: data.title,
          category: data.familiarSubCategory,
          date: serverTimestamp(),
          taskId: taskId // Link back
        });
      }
      ...
      if (data.financeInfo) {
        ...
        batch.set(financeRef, {
          ...
          pillar: data.financeInfo.pillar, // 'profissional' or 'casa'
          date: serverTimestamp(),
          taskId: taskId // Link back
        });
      }
      ...
      const { familiarSubCategory, financeInfo, ...coreData } = taskData as any;
      const cleanedTaskData = Object.fromEntries(
        Object.entries(coreData).filter(([_, v]) => v !== undefined)
      );

      batch.set(taskRef, cleanedTaskData);
      console.log("Tarefa preparada com ID:", taskId);
      ...
      await batch.commit();
      console.log("Tarefa criada com sucesso no Firestore. ID:", taskId);
  ...
  const handleUpgrade = async (planId?: string) => {
    ...
    if (institution?.id && planId) {
      try {
        await updateDoc(doc(db, 'institutions', institution.id), { planType: planId });
        toast.success(`Plano alterado para ${planId} com sucesso!`);
        return;
      } catch (error) {
  ...
  const effectiveUserData = userData || authUserData;
  const isMaster = user?.email === 'marcelle.gomesvieira.ayres@gmail.com';
  
  // Determine user categories with viewMode override for Master
  ...
  if (isPersonal) {
    ...
    sidebarItems.push({ id: 'institution', label: 'Gestão Pessoal', icon: Building2 });
  ...
  if (isMaster) {
    if (viewMode === null) {
      ...
      sidebarItems.push({ id: 'video-call', label: 'Chamada de Vídeo', icon: VideoIcon });
      sidebarItems.push({ id: 'admin-panel', label: 'Painel Master', icon: ShieldCheck });
    } else {
      // In specialized views, keep the Admin Panel as an "Escape Hatch" at the bottom
      // But strictly following "menu não corresponde ao que foi atualizado"
      // Suggests removing the overrides completely for true simulation.
      // I'll add the Painel Master only if it's not already restricted by the user's intent.
              ...
              {isMaster && (
                <div className="p-4 border-b bg-primary/5">
                  ...
                  <DropdownMenuTrigger render={
                    <Button variant="outline" className="w-full justify-between border-primary/20 bg-background rounded-xl">
                      <div className="flex items-center gap-2">
                        <Layers className="w-4 h-4 text-primary" />
                        <span className="text-xs font-bold uppercase">
                          {viewMode === 'PERSONAL' ? 'Visão Pessoal' : 
                          viewMode === 'INSTITUTION_OWNER' ? 'Visão Gestor' :
                          viewMode === 'INSTITUTION_MEMBER' ? 'Visão Membro' : 'Visão Auto'}
                        </span>
                      </div>
                      <ChevronDown className="w-4 h-4 opacity-50" />
                    </Button>
                  } />
                    <DropdownMenuContent className="w-[calc(288px-32px)] ml-4 rounded-2xl p-2">
                      <DropdownMenuGroup>
                        <DropdownMenuLabel className="text-[10px] uppercase font-black tracking-widest opacity-50 px-2 py-1.5 font-bold">Mudar Perspectiva</DropdownMenuLabel>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => setViewMode(null)} className="rounded-xl cursor-pointer gap-2 font-bold py-2.5">
                          <Zap className="w-4 h-4 text-primary" /> Automático
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => setViewMode('PERSONAL')} className="rounded-xl cursor-pointer gap-2 font-bold py-2.5">
                          <UserIcon className="w-4 h-4 text-blue-500" /> Visão Pessoal
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => setViewMode('INSTITUTION_OWNER')} className="rounded-xl cursor-pointer gap-2 font-bold py-2.5">
                          <Building2 className="w-4 h-4 text-purple-500" /> Visão Gestor (Admin)
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => setViewMode('INSTITUTION_MEMBER')} className="rounded-xl cursor-pointer gap-2 font-bold py-2.5">
                          <Users className="w-4 h-4 text-amber-500" /> Visão Membro (Equipe)
                        </DropdownMenuItem>
                      </DropdownMenuGroup>
                    </DropdownMenuContent>
                  </DropdownMenu>
        ...
        <header className={cn(
          ...
          <div className="flex items-center gap-3 md:gap-6 flex-1">
            ...
            <div className="relative max-w-sm w-full hidden sm:block">
              ...
              <input 
                ...
                className="w-full pl-10 pr-10 py-2 bg-muted/50 border-none rounded-full text-sm focus:ring-2 focus:ring-primary/20 outline-none transition-all"
              />
              {searchQuery && (
                <button 
          ...
          <div className="flex items-center gap-4">
            {isMaster && (
              <DropdownMenu>
                <DropdownMenuTrigger render={
                  <Button variant="outline" size="sm" className="hidden lg:flex gap-2 bg-primary/5 hover:bg-primary/10 border-primary/20 rounded-xl font-bold h-9">
                    <Layers className="w-4 h-4 text-primary" />
                ...
                <DropdownMenuContent align="end" className="w-56 rounded-2xl p-2">
                  <DropdownMenuGroup>
                    <DropdownMenuLabel className="text-[10px] uppercase font-black tracking-widest opacity-50 px-2 py-1.5">Mudar Perspectiva</DropdownMenuLabel>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem onClick={() => setViewMode(null)} className="rounded-xl cursor-pointer gap-2 font-bold py-2.5">
                      <Zap className="w-4 h-4 text-primary" /> Automático (Real)
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setViewMode('PERSONAL')} className="rounded-xl cursor-pointer gap-2 font-bold py-2.5">
                      <UserIcon className="w-4 h-4 text-blue-500" /> Visão Pessoal
                    </DropdownMenuItem>
          ...
          <AnimatePresence mode="wait">
            ...
            >
                  ...
                  <div className="grid grid-cols-1 gap-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {filteredTasks.filter(t => !t.completed).slice(0, 3).map(task => (
                          <div 
                            key={task.id} 
              ...
              {activeView === 'dashboard' ? (
                ...
                <PlanSelector 
                  currentPlan={institution?.planType}
